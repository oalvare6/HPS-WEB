...
1. **Email delivery for magic links (BLOCKER for the logged-out /pay flow).** No transactional provider exists in the repo (only Supabase Auth's own templates, which cannot carry an app link). Wire a `ResumeLinkSender` via `configureResumeLinkSender()` in `src/lib/email/resume-link-sender.ts` (or replace the unconfigured default) with whichever provider the operator chooses; until then requests are throttled, tokens are issued and expire unused, the response stays neutral, and **no link is delivered**. Signed-in players (`/register`, `/pay` server path) and players holding their own registration link are unaffected.
