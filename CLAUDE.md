...
| [`docs/STAGE-2-2-REPORT.md`](docs/STAGE-2-2-REPORT.md) | **Most recent session.** The isolated `hps-dev` project: built from the 41 migrations and verified object-by-object against the production catalog, seeded with synthetic data, and validated in SQL — the payment split, every waiver branch, the integrity constraints and the score math. Says plainly what is *not* proved (anything involving the UI), what the local bring-up found (§7: a key preflight that only checked the URL, and a verifier that reported its own parse bug as missing data), and the commands that close it. |
| [`docs/STAGE-2-3-PROPOSAL.md`](docs/STAGE-2-3-PROPOSAL.md) | **Proposed next.** Cash/Zelle receipts, the Resend send path, the cross-event team constraint and review reasons — with the one invariant question that has to be answered before any of it starts. Not begun. |
