...
> **Status 2026-09-11 — superseded.** Stage 2.2 is **complete** (44/44 through the running admin
> against `hps-dev`, [STAGE-2-2-REPORT.md](STAGE-2-2-REPORT.md)) and Stage 2.3 A, B, C and D are
> **built** ([STAGE-2-3-PROPOSAL.md](STAGE-2-3-PROPOSAL.md)), all on
> **`claude/dazzling-wozniak-es39bo`**. This page is the pre-2.2 handoff, kept as history; the
> sentences below that it overtook are marked in place. Production is still untouched and its
> migration ledger repair is still owed.

Working branch at the time: **`astra/stage-2-1-admin`** (now `claude/dazzling-wozniak-es39bo`). The owner authorized committing and pushing the current work for handoff only. Do not merge to `main`/Production, open a PR, deploy, or make database changes as part of this handoff.

## Read first

1. [STAGE-2-2-SETUP-CHECKLIST.md](STAGE-2-2-SETUP-CHECKLIST.md): exact isolated development setup, the migration filenames (41 then; 44 now, latest `20260911120000_message_batches.sql`), local environment values, synthetic seeding plan and acceptance checks.
...
## Stage 2.2 — only planned at this handoff; COMPLETE since 2026-09-11

~~No development Supabase project has been created or connected; no database has been migrated or seeded. No real database-backed Stage 2.2 workflow has been validated.~~ **Done:** `hps-dev` (`tfkdtwgxnumnuiiayrld`) exists, carries all 44 migrations, is seeded by `scripts/sql/stage22-seed.sql`, and the acceptance run passed 44/44 through the admin's own routes (commits 5e74c60 → f2cda3b). Stage 2.1 fixture checks must still not be reported as Stage 2.2 integration coverage — they are different things.
...
~~**Database approval is still pending.**~~ Database setup was approved and done. The rule survives: nothing here authorises touching Production (`jqkiswwunrnyqjgroqtn`).
...
Keep backend contracts, authentication, waiver authority, payment logic, schema design and event-state behavior fixed unless a concrete integration mismatch is reviewed within authorized scope. ~~General Resend communication and detailed Cash/Zelle receipts remain Stage 2.3 candidates.~~ Both shipped 2026-09-11 (Stage 2.3 B and A), as did the cross-event guard (C) and actionable review reasons (D).
...
- ~~The proposed `.env.stage22.local`, Stage 2.2 launcher and database seed runner do not exist yet.~~ They exist: `scripts/stage22-setup-env.ts`, `scripts/stage22-dev.ts` (the launcher; `--check` verifies the keys) and `scripts/sql/stage22-seed.sql` behind `scripts/stage22-guard.ts`. The custom env filename is still not automatically loaded by Next.js — the launcher loads it explicitly and refuses Production.
...
- `vercel.json` disables automatic Git deployments for **`astra/stage-2-1-admin` and `claude/dazzling-wozniak-es39bo`** to honor the owner's push-without-deploy instruction. Vercel's remote project settings and `main` are unchanged. Keep this guard for further pushes; another branch needs its own no-deploy review. [Vercel Git configuration](https://vercel.com/docs/project-configuration/git-configuration).
...
Build output included non-blocking cache-size and stale Browserslist-data notices. No dependencies were changed. ~~PostgreSQL, real Supabase and external-provider integration checks remain pending.~~ The PostgreSQL suites and the real-Supabase (`hps-dev`) checks have since run green (STAGE-2-2-REPORT.md, STAGE-2-3-PROPOSAL.md); external-provider checks (Stripe sandbox, DocuSeal callback, Google OAuth) remain pending.
